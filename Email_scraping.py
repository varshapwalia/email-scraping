...
with open('E:\downloads\Technology_startups.csv','r') as csvfile: #fetch data from csv
	plots=csv.reader(csvfile, delimiter=',')
	for row in plots:
		a.append(row[3]) #fill it in a list with the column that contains website urls
...
for temp in a:
	try:
		...
		for eachp in para:
			myfile = open("E:\email_list.csv", 'w')
			wr = csv.writer(myfile,delimeter=',')
			wr.writerow(eachp)
			print(eachp)
